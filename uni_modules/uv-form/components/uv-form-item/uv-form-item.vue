...
<script>
	 ...
	 * @tutorial https://www.uvui.com/components/form.html
	...
	export default {
		...
		methods: {
			init() {
				...
				if (!this.parent) {
					uni.$uv.error('uv-form-item需要结合uv-form组件使用')
				}
			},
			// 获取父组件的参数
			updateParentData() {
			...
			resetField() {
				// 找到原始值
				const value = uni.$uv.getProperty(this.parent.originalModel, this.prop)
				// 将uv-form的model的prop属性链还原原始值
				uni.$uv.setProperty(this.parent.model, this.prop, value)
				// 移除校验结果
				this.message = null
			},
			// 点击组件

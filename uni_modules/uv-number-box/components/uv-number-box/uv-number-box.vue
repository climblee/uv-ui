...
<script>
	 ...
	 * @tutorial https://uvui.com/components/numberBox.html
	...
	export default {
		...
		computed: {
			getCursorSpacing() {
				...
				return uni.$uv.getPx(this.cursorSpacing)
			},
			// 按钮的样式
			buttonStyle() {
				return (type) => {
					const style = {
						...
						height: uni.$uv.addUnit(this.buttonSize),
						color: this.color
					}
			...
			inputStyle() {
				...
				const style = {
					...
					backgroundColor: this.bgColor,
					height: uni.$uv.addUnit(this.buttonSize),
					width: uni.$uv.addUnit(this.inputWidth)
				}
				return style
			},
			// 用于监听多个值发生变化

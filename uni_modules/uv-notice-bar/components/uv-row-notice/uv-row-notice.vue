	...
	export default {
		...
		watch: {
			text: {
				...
				handler(newValue, oldValue) {
					...
					if(!uni.$uv.test.string(newValue)) {
						uni.$uv.error('noticebar组件direction为row时，要求text参数为字符串形式')
					}
				}
			},
			fontSize() {
		...
		methods: {
			init() {
				...
				if(!uni.$uv.test.string(this.text)) {
					uni.$uv.error('noticebar组件direction为row时，要求text参数为字符串形式')
				}
			},
			// vue版处理
			async vue() {
				...
				this.animationDuration = `${textWidth / uni.$uv.getPx(this.speed)}s`
			...
			loopAnimation(textWidth, boxWidth) {
				...
				animation.transition(this.$refs['uv-notice__content__text'], {
					...
					duration: (boxWidth + textWidth) / uni.$uv.getPx(this.speed) * 1000,
					delay: 10
				}, () => {
					animation.transition(this.$refs['uv-notice__content__text'], {
						styles: {
